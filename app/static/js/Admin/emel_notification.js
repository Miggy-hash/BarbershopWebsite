document.addEventListener('DOMContentLoaded', () => {
    ...
    let limit = 5; // Changed to 5 per batch
    let offset = 0;
    ...
    socket.on('slot_booked', (data) => {
        ...
        // Trim to max visible (but since we append older, this keeps newest)
    ...
    function updateNotifications(notifications, unreadCount) {
        ...
        if (loadedNotifications.length === 0) {
            ...
            if (showMoreBtn) {
                showMoreBtn.style.display = notifications.length >= offset + limit ? 'block' : 'none'; // Show if potentially more
            }
        }
    ...
    function fetchNotifications(append = false) {
        fetch(`/admin/emel-notifications/${limit}/${offset}`)
            ...
            .then(data => {
                ...
                if (append) {
                    loadedNotifications = [...loadedNotifications, ...data.notifications]; // Append older
                } else {
                    loadedNotifications = data.notifications; // Initial load
                }
                updateNotifications(loadedNotifications, data.unread_count);
                // Hide if no more (length < limit)
                if (data.notifications.length < limit && showMoreBtn) {
    ...
    if (showMoreBtn) {
        showMoreBtn.addEventListener('click', () => {
            offset += limit;
            fetchNotifications(true); // Append mode
        });
    }
    ...
    fetchNotifications(); // Initial load, append=false
}); 